...
export default function ProductDetails() {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${import.meta.env.VITE_APP_URL_BASE}/Product/${id}`, {
            method: "GET",
            headers: {
    ...
    <div className="pagesContainer">
      ...
      <div className="w-full flex items-center justify-between">
        <div>
          ...
          <h2 className="pagesTitle">product subName: {ProductData?.subName}</h2>
          <h2 className="pagesTitle">product size: {ProductData?.size}</h2>
          <h2 className="pagesTitle">product image: {ProductData?.image}</h2>
          <h2 className="pagesTitle">product boxItems: {ProductData?.boxItems}</h2>
          <h5 className="pagesTitle">brand name: {ProductData?.brand.name}</h5>
          <h5 className="pagesTitle">brand image: {ProductData?.brand.image}</h5>
        </div>
      </div>
    </div>
  );
